...
void hashtable_free(hashtable_t *tab)
{
    ...
    debug_print(DEBUG_DATASTRUCTURE, "free hashtable:\n");
    print_hashtable(tab);
    ...
    for(int i = 0; i < tab->num; i ++ )
    {
        ...
        {
            if(b->karray != NULL && b->varray[j] != NULL)
            {
                free(b->karray[j]);
                b->karray[j] = NULL;
            }
...
{
    uint64_t hid64 = has_function(key);
...
int hashtable_insert(hashtable_t **address, char *key, uint64_t value)
{
    ...
    {
        // full for this bucket's k-v array, expanding the whole
        if(b->localdepth == tab->globaldepth)
        {
            ...
            for(int i = 0; i < old_num; i ++ )
            {
                if(b != old_array[i])
                {
                    // this bucket is not changed - point to the same heap addressof bucket
...
static void split_bucket_full(hashtable_t *tab, hashtable_bucket_t *b)
{
    ...
    // hid64 now is the last hid64, but low bits should be the same
...
void print_hashtable(hashtable_t *tab)
{
    ...
    {
        hashtable_bucket_t *b = tab->directory[i];
        printf("[%d] local %d", i, b->localdepth, b);
        for(int j = 0; j < b->counter; j ++ )
        {
            rintf("%s   ", b->karray[j]);
        }
